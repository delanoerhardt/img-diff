...
class RandomShaker extends DiffFinder {

  def findDiffs(img1: BufferedImage, img2: BufferedImage): ArrayBuffer[LineState] = {
    val startLineAmmount = sqrt(img1.getHeight).toInt.max(1) + 2

    val rand = new Random()

    val list = new ArraySeq[Section](startLineAmmount)

    val buffer1 = new Array[Int](img1.getWidth)
    val buffer2 = new Array[Int](img2.getWidth)
    
    list(0) = new Section(0)
    list(1) = new Section(img1.getHeight-1)
    
    var currentPos = 0
    var changedPos = 0
    for(i <- (2 until startLineAmmount)) {
      list(i) = new Section(rand.nextInt(img1.getHeight))
    }

    for(i <- (0 until startLineAmmount)) {
